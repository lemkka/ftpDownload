...
unsigned long long ftp_list( int c_sock, char *path)
{
	...
	for(i=34;i<43;i++)
	{
		if(((char*)re_buf)[i]==' ') continue;
		fileLen=fileLen*10+((char*)re_buf)[i]-'0';
	}
...
int ftp_retrfile( Args* args)
{
	...
	char* s=args->s;
	char* d=args->d;
	unsigned long long totalLen=args->totalLen;
   ...
   fseek(handle, totalLen*index, SEEK_SET);
 
    //开始向PASV读取数据
	unsigned long long stor_size=0;
    bzero(buf, sizeof(buf));
    while ( (len = recv( d_sock, buf, 512, 0 )) > 0 ) {
      write_len = fwrite( buf, 1,len,handle );
	//printf("****%d %d\n",write_len,len);
        if (write_len != len )
        {
            close( d_sock );
    ...
    stor_size += write_len;
	// printf("%lld\n",stor_size);
	if(stor_size>=totalLen)
	{
		//ftp_sendcmd(c_sock,"ABOR");
		break;
	}
